...
public class ServiceCommentaire implements IService<Commentaire> {
    ...
    public void modifier(Commentaire commentaire) {
        try {
            String req = "UPDATE `commentaire` SET `id_user`='" + commentaire.getId_user() + "',`id_evenement`='" + commentaire.getEvenement().getId() + "',`commentaire`='" + commentaire.getCommentaire() + "',`date`='" + commentaire.getDate() + "' WHERE `commentaire`.`id` = " + commentaire.getId();
    ...
    public List<Commentaire> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                Commentaire commentaire = new Commentaire(rs.getInt(1),rs.getInt(2),se.getOneById((rs.getInt(3))),rs.getString(4),rs.getDate(5));
    ...
    public Commentaire getOneById(int id) {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery(req);
            while (rs.next()) {
                commentaire = new Commentaire(rs.getInt(1),se.getOneById(rs.getInt(2)),rs.getString(3),rs.getDate(4));
